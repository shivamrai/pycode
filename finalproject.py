...
def get_fifa_stats(match, player_stats):
    ...
    for player in players:
        ...
        # Aggregate stats
        player_stats_new = pd.concat(
            [player_stats_new, overall_rating], axis=1)
...
def get_fifa_data(matches, player_stats, path=None, data_exists=False):
    ...
    if data_exists:
        ...
        fifa_data = matches.apply(
            lambda x: get_fifa_stats(
                x, player_stats), axis=1)

        end = time()
        print(
            "Fifa data collected in {:.1f} minutes".format(
                (end - start) / 60))
...
def get_last_matches_against_eachother(
        matches, date, home_team, away_team, x=10):
...
def get_match_features(match, matches, x=10):
    ...
    result.loc[0, "games_won_home_team"] = get_wins(
        matches_home_team, home_team)
    result.loc[0, "games_won_away_team"] = get_wins(
        matches_away_team, away_team)
    result.loc[0, "games_against_won"] = get_wins(
        last_matches_against, home_team)
    result.loc[0, "games_against_lost"] = get_wins(
...
def create_feables(
    ...
    match_stats = matches.apply(
        lambda x: get_match_features(
            x, matches, x=10), axis=1)
    ...
    if verbose:
        print(
            "Match features generated in {:.1f} minutes".format(
                (end - start) / 60))
    ...
    if verbose:
        print(
            "Match labels generated in {:.1f} minutes".format(
                (end - start) / 60))
    ...
    if verbose:
        print(
            "Bookkeeper data generated in {:.1f} minutes".format(
                (end - start) / 60))
...
def explore_data(features, inputs, path):
    ...
    for col in features.columns:
        ...
        sns.distplot(inputs[inputs["label"] ==
                            "Win"].iloc[:, j], hist=False, label="Win")
        sns.distplot(inputs[inputs["label"] ==
                            "Draw"].iloc[:, j], hist=False, label="Draw")
        sns.distplot(inputs[inputs["label"] ==
                            "Defeat"].iloc[:, j], hist=False, label="Defeat")
        plt.legend()
        i = i + 1
...
for i in range(num_features):
    ...
    score = np.mean(cross_val_score(
        R_F, X[col].values.reshape(-1, 1), y, cv=10))
